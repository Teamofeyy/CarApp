      ...
      >
        {onboarding.map((item) => (
          <View key={item.id} className="flex items-center justify-center p-5">
            ...
            <View className='flex flew-row items-center justify-center w-full mt-10'>
              <Text className='text-black text-3xl font-bold mx-10'>{item.title}</Text>
            </View>
            <Text className='text-medium font-JakartaSemiBold text-center text-[#858585] mx-10 mt-3 text-lg'>
              {item.description}
            </Text>
          </View>
        ))}
